...
test("all release and runtime version surfaces are exactly 0.3.0", async () => {
  ...
  assert.equal(marketplaceFleet.version, "0.3.0");
  assert.equal(plugin.version, "0.3.0");
  assert.match(pkg.scripts["package:plugin"], /--version 0\.3\.0$/u);
  assert.match(pkg.scripts["release:check"], /--version 0\.3\.0$/u);
  assert.match(broker, /CLIENT_INFO[\s\S]*version: "0\.3\.0"/u);
  assert.doesNotMatch(packager, /0\.1\.7/u);
  assert.equal((packager.match(/0\.3\.0/gu) ?? []).length, 2);
});
