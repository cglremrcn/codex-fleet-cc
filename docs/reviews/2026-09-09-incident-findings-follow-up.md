...
This follow-up owns the operator surface that makes those runtime capabilities usable without reading source code or editing Fleet state by hand. It intentionally does not widen the Codex sandbox, fabricate a PostgreSQL target, vendor application fonts, or treat a missing browser as visual proof.
...
| Windows nested-process `EPERM` / Python worktree provenance | PR #14 preflights before the first model turn and blocks false-green execution. Host policy is not weakened. |
...
| Plugin/runtime version path mismatch | PR #15's CLI entrypoint resolves the ownership-manifest integration runtime for control commands; setup/uninstall remain on the installed plugin surface. |
...
Likewise, a sandbox that cannot spawn the application's required nested worker is not made "green" by weakening Fleet's authority boundary. The P0 behavior is to discover that before spending a model turn and hand the exact controller-owned verification boundary back explicitly.
